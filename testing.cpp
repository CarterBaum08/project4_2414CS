...
void quicksort_rows_rec(int* data, int lo, int hi, int C,
    ...
    int pivot = partition_lomuto(data,lo,hi,C,cmpCount,exchanges);
    quicksort_rows_rec(data,lo, pivot-1,C,cmpCount,exchanges);
    quicksort_rows_rec(data,pivot+1,hi,C,cmpCount,exchanges);
}
...
void heapify(int* data, int n, int i, int C, long long &cmpCount, long long &exchanges) {//i is the parent element
  // TODO
...
//fix and read over what it does
void lexsort_lexpass(int* data, int R, int C,
   ...
   for(int col =C-1;col>=0;col--)
   {
    for(int i=0; i<R;i++)
    {
      int* TEMP = new int[C] ;//AI suggestion
